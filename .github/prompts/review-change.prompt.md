...
2. Read every changed path and hunk, including any tool-managed overflow output. If the diff command fails, report the error and stop without replacing an existing report. If output is incomplete, identify the coverage gap in chat and stop without replacing an existing report.
